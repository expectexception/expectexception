...
FEATURE_FLAGS = {
    ...
    "HEAVY_AI_SERVICES": os.getenv("FEATURE_HEAVY_AI_ENABLED", "False").lower()
    in ("true", "1", "yes"),
    "YT_DOWNLOADER": os.getenv("FEATURE_YT_DOWNLOADER_ENABLED", "False").lower()
    in ("true", "1", "yes"),
    "CELERY_BEAT": os.getenv("FEATURE_CELERY_BEAT_ENABLED", "False").lower()
    in ("true", "1", "yes"),
}
...
# Overridable so CI/tests (Django's test client speaks plain HTTP) don't get
# redirected on every request. Render itself always sets this True.
