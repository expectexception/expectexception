...
class AuthTests(APITestCase):
    def test_register_and_login(self):
        ...
        r2 = self.client.post(
            login_url, {"email": "test@example.com", "password": "StrongPass123!"}, format="json"
        )
    ...
    def test_password_reset_flow(self):
        ...
        # The link format we send uses query params.
        ...
        r2 = self.client.post(
            confirm_url, {"uid": uid, "token": token, "new_password": "NewPass123!"}, format="json"
        )
        self.assertEqual(r2.status_code, status.HTTP_200_OK)
...
class MongoFailoverLoginTests(APITestCase):
    ...
    def test_failed_login_does_not_overwrite_existing_local_user(self):
        ...
        self.assertEqual(
            user.password, original_hash, "local password hash was overwritten from the mirror"
        )
...
class LogoutTests(APITestCase):
    ...
    def test_logout_revokes_refresh_token(self):
        ...
        self.assertEqual(
            self.client.post(
                reverse("auth_logout"), {"refresh": refresh}, format="json"
            ).status_code,
            status.HTTP_200_OK,
        )
