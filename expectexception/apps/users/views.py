...
class EmailClaimTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    def _authenticate_from_mongo_mirror(email, password):
        ...
        logger.info("JIT-created shadow user %s from Mongo mirror during login (failover path).", email)
        return user
...
class LogoutView(APIView):
    ...
    def post(self, request):
        ...
        if not refresh:
            return Response({'detail': 'Provide refresh token.'}, status=status.HTTP_400_BAD_REQUEST)
        try:
            ...
            logger.warning("Token blacklist app is not installed; logout could not revoke the refresh token.")
