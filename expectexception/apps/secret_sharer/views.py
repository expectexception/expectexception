...
from .utils import decrypt_content, encrypt_content
...
class CreateSecretView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            secret = Secret.objects.create(encrypted_content=encrypted, expires_at=expires_at)
...
class ViewSecretView(APIView):
    ...
    def get(self, request, pk):
        try:
            ...
            return Response(
                {"error": "Secret not found or already viewed"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        try:
            ...
            return Response(
                {"error": "Decryption failed"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
from .utils import decrypt_bytes, encrypt_bytes
...
class CreateFileSecretView(APIView):
    ...
    def post(self, request):
        ...
        if file_obj.size > 50 * 1024 * 1024:
            return Response(
                {"error": "File too large. Limit is 50MB."}, status=status.HTTP_400_BAD_REQUEST
            )
...
class DownloadFileSecretView(APIView):
    ...
    def get(self, request, pk):
        try:
            ...
            return Response(
                {"error": "File not found or already downloaded"}, status=status.HTTP_404_NOT_FOUND
            )
