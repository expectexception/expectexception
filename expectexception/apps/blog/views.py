...
class PostViewSet(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, *args, **kwargs):
        try:
            ...
            doc = (
                find_in_mongo("blog_posts", int(raw_id))
    ...
    def bookmark(self, request, pk=None):
        ...
        if request.method == "POST":
            ...
            if not created:
                return Response(
                    {"detail": "Already bookmarked"}, status=status.HTTP_400_BAD_REQUEST
                )
    ...
    def restore_revision(self, request, pk=None):
        ...
        if post.author != request.user:
            return Response(
                {"detail": "Only author can restore revisions"}, status=status.HTTP_403_FORBIDDEN
            )
    ...
    def analytics(self, request, pk=None):
        ...
        if not request.user.is_authenticated or (
            post.author != request.user and not request.user.is_staff
        ):
            return Response({"detail": "Permission denied"}, status=status.HTTP_403_FORBIDDEN)
