...
def optimize_image(image_file, max_width=1200, max_height=1200, quality=85):
    ...
    try:
        ...
        output = BytesIO()
        img.save(
            output, format="WEBP", quality=quality, method=4
        )  # method=4 is a good balance of speed/compression
        output.seek(0)
...
def sanitize_html(content):
    ...
    return clean(
        content, tags=allowed_tags, attributes=allowed_attributes, styles=allowed_styles, strip=True
    )
...
def auto_save_draft(author, title="", content="", post=None):
    ...
    if post:
        draft, created = PostDraft.objects.get_or_create(
            post=post, author=author, defaults={"title": title, "content": content}
        )
    else:
        # Get most recent draft for this author without a post
        draft = PostDraft.objects.filter(author=author, post__isnull=True).first()

        if not draft:
            draft = PostDraft.objects.create(author=author, title=title, content=content)
            created = True
        else:
            created = False
