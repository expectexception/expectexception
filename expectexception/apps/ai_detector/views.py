...
def analyze_image(request):
    ...
    try:
        ...
        logger.info(
            f"Analysis complete for {filename}: {results.get('label', 'Unknown')} ({confidence_val:.2f}%)"
        )
        ...
        return Response(
            {"error": f"Analysis failed: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
    finally:
...
def get_task_status(request, task_id):
    ...
    try:
        ...
        return Response(
            {"error": "Async processing not available"}, status=status.HTTP_503_SERVICE_UNAVAILABLE
        )
    except Exception as e:
...
def health_check(request):
    ...
    try:
        ...
        try:
            ...
            importlib.import_module(".tasks", package=__package__)
