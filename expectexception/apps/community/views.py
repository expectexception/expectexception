...
from .models import Category, Reply, Thread, ThreadBookmark, Vote
...
class ThreadViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        qs = Thread.objects.select_related("author", "category").order_by(
            "-is_pinned", "-last_activity"
        )
    ...
    def retrieve(self, request, *args, **kwargs):
        try:
            ...
            doc = (
                find_in_mongo("community_threads", int(raw_id))
...
class ReplyViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        Thread.objects.filter(pk=reply.thread_id).update(
            reply_count=reply.thread.reply_count + 1, last_activity=timezone.now()
        )
    ...
    def accept(self, request, pk=None):
        ...
        Reply.objects.filter(thread=thread, is_accepted_answer=True).update(
