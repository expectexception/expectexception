...
class SubscribeView(views.APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"error": "Failed to subscribe"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class UnsubscribeView(views.APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"error": "Failed to unsubscribe"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class VapidPublicKeyView(views.APIView):
    ...
    def get(self, request):
        ...
        if not vapid_key:
            return Response(
                {"error": "VAPID key not configured"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
        ...
        data = [
            {
                ...
                "sender": (
                    getattr(n.sender, "first_name", None) or getattr(n.sender, "email", None)
                    if n.sender
                    else None
                ),
            }
