...
def generate_handwriting_image(
    text, font_name="caveat", paper_type="plain", ink_color="blue", font_size=32
):
    """
    ...
    if paper_type == "lined":
        ...
        # Draw margin line
        draw.line(
            [(margin_left - 10, 0), (margin_left - 10, page_height)], fill=(255, 200, 200), width=2
        )
    ...
    for _line_idx, line in enumerate(wrapped_lines):
        ...
        for char in line:
            ...
            char_draw.text((10, 10), char, font=font, fill=fill_color + (255,))

            # Rotate character
            try:
                rotated_char = char_img.rotate(
                    char_angle, expand=True, resample=Image.Resampling.BILINEAR
                )
            except Exception:
