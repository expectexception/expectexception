    ...
    try:
        ...
        try:
            ...
            LogIntelligence.objects.update_or_create(
                date=timezone.now().date(), defaults={"state_data": state}
            )
        ...
        avg_durations = [
            {"path": p, "avg": a["sum"] / a["count"], "count": a["count"]}
            for p, a in durations_agg.items()
        ]
        ...
        return {
            ...
            "parsed_lines": parsed_lines,
            "top_ips": [
                {"ip": ip, "count": count, "geo": geo_cache.get(ip, {})}
            ...
            "status_distribution": [
                {
                    "status": s,
                    "count": c,
                    "percentage": round((c / parsed_lines) * 100, 1) if parsed_lines > 0 else 0,
                }
            ...
            "countries": [
                {
                    "country": k,
                    "count": v,
                    "percentage": round((v / parsed_lines) * 100, 1) if parsed_lines > 0 else 0,
                }
            ...
            "hourly_series": {
                "labels": [x[0] for x in sorted(hourly_hits.items())],
                "data": [x[1] for x in sorted(hourly_hits.items())],
            },
            "path_errors": [
                {"path": k, "total": sum(v.values()), "breakdown": dict(v)}
                for k, v in sorted(
                    path_errors.items(), key=lambda x: sum(x[1].values()), reverse=True
                )[:10]
            ],
