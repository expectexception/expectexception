...
from .models import DownloadableResource, Service, ToolUsage, UserToolRestriction
    ...
    def patch(self, request, pk):
        ...
        if user == request.user:
            if "is_active" in request.data and not request.data["is_active"]:
                return Response(
                    {"error": "Cannot deactivate yourself"}, status=status.HTTP_400_BAD_REQUEST
                )
        ...
        try:
            if "email" in request.data:
                ...
                if not new_email:
                    return Response(
                        {"error": "Email cannot be empty"}, status=status.HTTP_400_BAD_REQUEST
                    )
...
class AdminInquiryReplyView(APIView):
    ...
    def post(self, request, pk):
        ...
        try:
            ...
            return Response(
                {"error": f"Failed to send email: {e}"}, status=status.HTTP_502_BAD_GATEWAY
            )
...
class OllamaModelsView(APIView):
    ...
    def get(self, request):
        try:
                ...
                return Response(
                    {
                        ...
                        "details": "Ollama server is offline or base URL is incorrect.",
                    },
                    status=503,
                )
            ...
            return Response({"models": models, "count": len(models), "active_model": active_model})
        except Exception as e:
...
class OllamaModelControlView(APIView):
    ...
    def post(self, request):
        ...
        try:
            if action == "pull":
                ...
                return Response(
                    {"message": f"Deleted {model_name}", "action": "delete", "model": model_name}
                )
                ...
                try:
                    ...
                    return Response(
                        {"error": f"Failed to load model {model_name}: {str(e)}"}, status=500
                    )
                ...
                try:
                    ...
                    return Response(
                        {"message": "Ollama service restarted successfully", "action": "restart"}
                    )
                except Exception as e:
    ...
    def get(self, request):
        ...
        return Response({"active_model": active, "has_active": bool(active)})
...
class OllamaStatusView(APIView):
    ...
    def get(self, request):
        try:
            ...
            if running:
                ...
                for rm in raw_running:
                    ...
                    running_models.append(
                        {"name": name, "is_active": name == self._get_active_model()}
                    )
            ...
            return Response(
                {"running": False, "error": str(e), "active_models": [], "active_model": None}
            )
...
class AdminToolRestrictionListCreateView(APIView):
    ...
    def get(self, request):
        restrictions = UserToolRestriction.objects.select_related(
            "user", "service", "restricted_by"
        ).all()
        data = [
            {
                "id": r.id,
    ...
    def post(self, request):
        ...
        if not user_id or not service_id:
            return Response(
                {"error": "user_id and service_id are required"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if not created:
            return Response(
                {"error": "This user is already restricted from this tool"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class AdminUserToolUsageView(APIView):
    ...
    def get(self, request, pk):
        ...
        restricted_services = {
            s.id: s.title for s in Service.objects.filter(id__in=restricted_service_ids)
        }
        ...
        return Response(
            {
                ...
                "restrictions": [
                    {"service_id": sid, "service_title": title}
                    for sid, title in restricted_services.items()
                ],
            }
        )
        ...
        return Response(
            {
                ...
                "top_tools": [
                    {"tool_name": t["tool_name"], "total_calls": t["total_calls"]}
                    for t in top_tools
                ],
                "daily_trend": [
                    {"date": d["day"].isoformat(), "total_calls": d["total_calls"]}
