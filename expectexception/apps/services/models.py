...
class Service(models.Model):
    ...
    requires_login = models.BooleanField(
        default=False, help_text="If checked, only authenticated users can use this tool"
    )
...
class DownloadableResource(models.Model):
    ...
    slug = models.SlugField(
        max_length=255, unique=True, blank=True, help_text="URL-friendly identifier"
    )
    seo_title = models.CharField(
        max_length=100, blank=True, help_text="Custom SEO title (optional)"
    )
    seo_description = models.CharField(
        max_length=160, blank=True, help_text="SEO meta description (optional)"
    )
...
class ToolUsage(models.Model):
    ...
    tool_name = models.CharField(
        max_length=120, db_index=True, help_text="Logical tool name e.g. 'qr-generator'"
    )
    endpoint = models.CharField(
        max_length=255, blank=True, help_text="Request path e.g. /api/services/qr-generator/"
    )
    ...
    def __str__(self):
        user_label = (
            getattr(self.user, "email", None) or getattr(self.user, "username", None) or "Anonymous"
        )
...
class DownloadHistory(models.Model):
    ...
    duration_seconds = models.FloatField(
        null=True, blank=True, help_text="How long the download took"
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class WebhookEndpoint(models.Model):
    ...
    secret = models.CharField(
        max_length=128, blank=True, help_text="Optional secret for validating incoming requests"
    )
...
class UptimeMonitor(models.Model):
    ...
    target = models.CharField(
        max_length=500, blank=True, help_text="URL or hostname; not used for heartbeat monitors"
    )
...
class SeoKeywordOverride(models.Model):
    ...
    title = models.CharField(
        max_length=200, blank=True, help_text="Leave blank to keep the page's default title"
    )
    description = models.TextField(
        blank=True, help_text="Leave blank to keep the page's default description"
    )
