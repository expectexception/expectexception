...
class Post(models.Model):
    ...
    seo_title = models.CharField(
        max_length=70, blank=True, help_text="Optimize for search engines (Max 70 chars)"
    )
    seo_description = models.CharField(
        max_length=160, blank=True, help_text="Meta description (Max 160 chars)"
    )
    ...
    table_of_contents = models.JSONField(
        default=list, blank=True, help_text="Auto-generated table of contents"
    )
    series = models.ForeignKey(
        "PostSeries", on_delete=models.SET_NULL, null=True, blank=True, related_name="posts"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if is_new:
            Post.objects.filter(pk=self.post_id).update(
                bookmarks_count=models.F("bookmarks_count") + 1
            )
...
class PostDraft(models.Model):
    ...
    post = models.ForeignKey(
        Post, on_delete=models.CASCADE, related_name="drafts", null=True, blank=True
    )
