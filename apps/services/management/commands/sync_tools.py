    ...
    def handle(self, *args, **kwargs):
        ...
        for tool in tools:
            ...
            defaults = {
                ...
                "tags": tool[
                    "tags"
                ],  # Django Arrayfield handles list directly if configured, else needs CSV
                "is_active": True,
            }

            obj, created = Service.objects.update_or_create(
                path=tool["path"], defaults=defaults  # Use path as unique identifier
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Successfully synced tools. Created: {created_count}, Updated: {updated_count}"
            )
        )
