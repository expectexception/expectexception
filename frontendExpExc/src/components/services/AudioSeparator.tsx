...
const AudioSeparator: React.FC = () => {
    ...
    return (
        <ServicePageShell
            ...
            maxWidth="sm"
            about="Splits an uploaded audio file into two isolated stems - vocals and instrumental - using our AI source-separation engine, run on our GPU server rather than in your browser. Upload an MP3, WAV, FLAC, or M4A file up to 50MB and the server processes it as a background job, since separation is computationally heavy; when it finishes you get two WAV stems to preview and download, plus a combined ZIP. Handy for making karaoke tracks, isolating vocals for sampling, or pulling an instrumental from a mix that was never released separately."
            howToSteps={[
            ...
            faq={[
                { question: 'What technology does the separation?', answer: 'A specialized AI audio-separation engine, run server-side with GPU acceleration rather than in your browser - that\'s why processing takes real time instead of being instant.' },
                { question: "Why does it only produce two stems instead of splitting out drums, bass, etc separately?", answer: 'This tool runs in two-stem mode - vocals versus everything else - rather than full instrument-by-instrument separation, which keeps processing faster and the output simpler: one vocal track, one backing track.' },
