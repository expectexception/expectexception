...
const ImageToText: React.FC = () => {
    ...
    return (
        <ServicePageShell
            ...
            maxWidth="md"
            about="Image to Text (OCR) extracts machine-readable text from a photo or screenshot using our OCR engine running on the server, the image is sent to the backend, read directly in memory, and the extracted text is returned; the image itself is never written to disk. The Language dropdown lists whatever languages are currently supported, so the exact list can vary between deployments; if you pick a language that isn't available, the backend automatically falls back to English and shows a warning."
            howToSteps={[
            ...
            faq={[
                { question: 'Which languages are supported?', answer: "The dropdown only lists what's currently available on the server. If you request a language that isn't available, the tool falls back to English and shows a warning." },
